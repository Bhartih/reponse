...
function InsertConnector(divid,rad=gridConnectorRadius){
	...
	console.log("DivId from InsertConnector : "+divid+" :: Num of bits from InsertConnector(not required kindly check) : "+numofbits);
	console.log("parent : "+divid);
	var divWidth=$("#"+divid).width();
	var divHeight=$("#"+divid).height();
	var cx=divWidth/2;
	var cy=divHeight/(2);  //if we remove term numofbits from here thing work fine to be checked.
	if(divid!="body"){
		var svgContainer = d3.select("#"+divid).append("svg")
								 .attr("width",divWidth)
								 .attr("height",divHeight)
